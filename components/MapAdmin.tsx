...
function MapCard({ map, active, onSelect, onTest, onDelete }: { map: TrackDef; active: boolean; onSelect: () => void; onTest: () => void; onDelete: () => void }) {
  return (
    <article onClick={onSelect} className={`group relative cursor-pointer overflow-hidden rounded-2xl border p-4 transition ${active ? "border-amber-400/70 bg-amber-400/[0.08] shadow-[0_0_30px_rgba(224,168,78,0.08)]" : "border-white/10 bg-white/[0.03] hover:border-white/25"}`}>
      ...
      <div className="flex gap-4"><div className="w-28 shrink-0">{map.gates.length > 0 ? <RoutePreview points={map.gates.map((gate) => gate.position)} compact accent={map.accent} /> : <div className="flex aspect-square items-center justify-center rounded-xl border border-white/10 bg-[#1b1209] font-mono text-[10px] uppercase tracking-[0.18em] text-amber-300/60">자유 주행</div>}</div><div className="min-w-0 flex-1"><div className="mb-1 flex items-center gap-2"><h3 className="truncate font-heading text-lg font-bold uppercase">{map.name}</h3></div><p className="line-clamp-2 min-h-8 text-xs text-white/40">{map.blurb}</p><div className="mt-2 flex gap-3 font-mono text-[9px] uppercase text-white/35">{map.gates.length > 0 ? <><span>체크포인트 {map.gates.length}개</span><span>{map.defaultLaps}바퀴</span></> : <span>체크포인트 없음</span>}</div></div></div>
      <div className="mt-3 flex gap-2 border-t border-white/5 pt-3"><button type="button" onClick={(event) => { event.stopPropagation(); onTest(); }} className="flex-1 rounded-lg bg-amber-400 px-3 py-2 font-heading text-xs font-medium uppercase tracking-wider text-[#2a1608] transition hover:bg-amber-300">테스트 주행</button><button type="button" aria-label={`${map.name} 삭제`} onClick={(event) => { event.stopPropagation(); onDelete(); }} className="rounded-lg border border-white/10 px-3 text-sm text-white/40 transition hover:border-red-400/40 hover:text-red-300">×</button></div>
    </article>
  );
}
